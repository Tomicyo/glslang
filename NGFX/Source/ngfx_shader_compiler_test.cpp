#include "../Public/ngfx_shader_compiler.h"
#include <memory>

using namespace ngfx;

extern const char *raygen;

int main(int argc, const char *argv) { 
    IShaderCompiler *pCompiler = nullptr;
    ICompilerResult *pResult = nullptr;
    ngfxCreateGlslangCompiler(&pCompiler);

    CompileOptions option;
    option.EntryPoint = "RayGen";
    option.ShaderType = ShaderStageBit::Raygen;
    pCompiler->Compile(raygen, "simple.hlsl", option, nullptr, nullptr, &pResult);

    ngfxDestroyGlslangCompiler(pCompiler);
    return 0; 
}

const char *raygen = R"(struct HitInfo
{
